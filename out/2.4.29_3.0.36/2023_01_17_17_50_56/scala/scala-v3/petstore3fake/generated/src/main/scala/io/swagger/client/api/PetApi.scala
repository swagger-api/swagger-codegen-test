...
class PetApi(
   ...
   */
  def getAllPets(): Option[AllPetsResponse] = {
    val await = Try(Await.result(getAllPetsAsync(), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * 
   *
   * @return Future(AllPetsResponse)
   */
  def getAllPetsAsync(): Future[AllPetsResponse] = {
      helper.getAllPets()
  }
   ...
   */
  def getRandomPet(): Option[SinglePetResponse] = {
    val await = Try(Await.result(getRandomPetAsync(), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * 
   *
   * @return Future(SinglePetResponse)
   */
  def getRandomPetAsync(): Future[SinglePetResponse] = {
      helper.getRandomPet()
  }
...
class PetApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getAllPets()(implicit reader: ClientResponseReader[AllPetsResponse]): Future[AllPetsResponse] = {
    // create path and map variables
    val path = (addFmt("/allPets"))
  ...
  def getRandomPet()(implicit reader: ClientResponseReader[SinglePetResponse]): Future[SinglePetResponse] = {
    // create path and map variables
    val path = (addFmt("/randomPet"))
