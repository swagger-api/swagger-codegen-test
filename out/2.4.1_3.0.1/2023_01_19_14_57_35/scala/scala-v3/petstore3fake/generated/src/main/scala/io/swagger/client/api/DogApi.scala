...
class DogApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def addDog(body: )(implicit reader: ClientResponseReader[Unit], writer: RequestWriter[]): Future[Unit] = {
  ...
  def updateDog(body: )(implicit reader: ClientResponseReader[Unit], writer: RequestWriter[]): Future[Unit] = {
    // create path and map variables
    val path = (addFmt("/dog"))
