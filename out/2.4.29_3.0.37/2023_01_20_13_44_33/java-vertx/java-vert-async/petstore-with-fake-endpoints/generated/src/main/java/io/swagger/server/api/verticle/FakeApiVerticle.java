    ...
    public void start() throws Exception {
        ...
        vertx.eventBus().<JsonObject> consumer(FAKEOUTERBOOLEANSERIALIZE_SERVICE_ID).handler(message -> {
            try {
                ...
                service.fakeOuterBooleanSerialize(body, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "fakeOuterBooleanSerialize");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(FAKEOUTERCOMPOSITESERIALIZE_SERVICE_ID).handler(message -> {
            try {
                ...
                service.fakeOuterCompositeSerialize(body, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "fakeOuterCompositeSerialize");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(FAKEOUTERNUMBERSERIALIZE_SERVICE_ID).handler(message -> {
            try {
                ...
                service.fakeOuterNumberSerialize(body, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "fakeOuterNumberSerialize");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(FAKEOUTERSTRINGSERIALIZE_SERVICE_ID).handler(message -> {
            try {
                ...
                service.fakeOuterStringSerialize(body, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "fakeOuterStringSerialize");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTBODYWITHQUERYPARAMS_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testBodyWithQueryParams(body, query, result -> {
                    if (result.succeeded())
                        message.reply(null);
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testBodyWithQueryParams");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTCLIENTMODEL_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testClientModel(body, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testClientModel");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTENDPOINTPARAMETERS_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testEndpointParameters(number, _double, patternWithoutDelimiter, _byte, integer, int32, int64, _float, string, binary, date, dateTime, password, paramCallback, result -> {
                    if (result.succeeded())
                        message.reply(null);
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testEndpointParameters");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTENUMPARAMETERS_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testEnumParameters(enumFormStringArray, enumFormString, enumHeaderStringArray, enumHeaderString, enumQueryStringArray, enumQueryString, enumQueryInteger, enumQueryDouble, result -> {
                    if (result.succeeded())
                        message.reply(null);
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testEnumParameters");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTINLINEADDITIONALPROPERTIES_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testInlineAdditionalProperties(param, result -> {
                    if (result.succeeded())
                        message.reply(null);
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testInlineAdditionalProperties");
                    }
                });
            } catch (Exception e) {
        ...
        vertx.eventBus().<JsonObject> consumer(TESTJSONFORMDATA_SERVICE_ID).handler(message -> {
            try {
                ...
                service.testJsonFormData(param, param2, result -> {
                    if (result.succeeded())
                        message.reply(null);
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "testJsonFormData");
                    }
                });
            } catch (Exception e) {
