...
public interface UserApiDelegate {
    ...
    default ResponseEntity<User> getUserByName( String  username) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            ...
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("{\"empty\": false}", User.class), HttpStatus.NOT_IMPLEMENTED);
                } catch (IOException e) {
    ...
    default ResponseEntity<String> loginUser( String  username,
        ...
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            ...
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("{  \"bytes\": [],  \"empty\": true}", String.class), HttpStatus.NOT_IMPLEMENTED);
                } catch (IOException e) {
                    log.error("Couldn't serialize response for content type application/json", e);
                    return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
                }
